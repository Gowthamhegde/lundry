...
import { Menu, Moon, Shirt, Sun, X, LogOut, LogIn } from "lucide-react";
...
export default function Navbar() {
  ...
  const visibleLinks = authed ? [...publicLinks, ...authLinks] : publicLinks;
              ...
              return (
                <Link key={nav.path} href={nav.path}
                  className={`relative px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 group ${
                    isActive ? "text-teal-700 dark:text-teal-300" : "text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100"
                  }`}
                >
                  {isActive && <span className="absolute inset-0 rounded-full bg-teal-50 dark:bg-teal-500/10 ring-1 ring-teal-200/60 dark:ring-teal-500/20" />}
                  ...
                  {isActive && <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-teal-500" />}
                </Link>
              );
            })}
          </div>
            ...
            return (
              <Link key={nav.path} href={nav.path}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-200 ${
                  isActive ? "bg-teal-50 dark:bg-teal-500/10 text-teal-700 dark:text-teal-300"
